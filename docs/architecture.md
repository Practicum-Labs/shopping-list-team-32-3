...
| Слой | Решение |
|---|---|
| UI | **Jetpack Compose** (Kotlin 2.2, плагин Compose) |
| Архитектура | **MVI** в `ui` + слои `data / domain / ui` |
| База данных | **Room** |
| DI | **Koin 4** (`koin-android`, `koin-compose`, `koin-compose-viewmodel`) |
| Навигация | **navigation-compose**, type-safe маршруты (`@Serializable`) |
| Сеть | Не используется в текущей версии. Если появятся сетевые запросы — **Retrofit**. |
| Хранение | Все данные приложения сохраняются в локальной БД (Room) |
